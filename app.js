...
const steps = [
  ...
  { title: "내 준비 현황", description: "오늘 할 일 확인" },
...
const APP_I18N = {
  ...
  zh: {
    ...
    readiness: {
      eyebrow: "准备情况确认", title: "请告诉我们您目前已经准备好的项目。", lead: "不确定的项目可以不勾选，结果页面会提示可能需要补充的材料。", subEyebrow: "账户·保险信息", subTitle: "这些信息会用于我的进度和保险管理页面。", subLead: "如果暂时不知道，可以留空，之后也可以修改。", departure: "预计出境日期（不确定可留空）", accountTitle: "我已经开立银行账户", accountDesc: "如果已经选择银行并完成开户，请勾选。", oneYearTitle: "我已在或预计在当前单位工作满1年", oneYearDesc: "这会影响出国满期保险是否可由本人直接申请。",
...
/* Gemini 응답에 포함된 가벼운 마크다운(**굵게**, 줄바꿈)만 안전하게 렌더링한다.
   먼저 HTML 이스케이프를 해서 XSS를 막은 다음, 그 결과 문자열 위에 **...** → <strong>,
   줄바꿈 → <br> 치환을 적용한다(이스케이프 후에는 <, > 같은 문자가 남지 않으므로 안전). */
...
function aiMessagesHtml() {
  ...
  return messages.map((message) => `<div class="ai-message ${message.role}${message.loading ? " loading" : ""}">${formatAiText(message.text)}</div>`).join("");
}
  ...
  return `
    <article class="ins-card">
      ...
      <div class="qa ${qaOpen}" id="qa-${kind}">
        ...
        ${qa ? `<div class="q">Q. ${escapeAiHtml(qa.q)}</div><div class="a ${qa.refuse ? "refuse" : ""}">${formatAiText(qa.text)}</div>` : ""}
        <div class="presets">
